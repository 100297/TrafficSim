...
public class Start extends JPanel implements Runnable {
	...
	// Data display variables (doesnt have anything to do with the running of the
	// program)
		...
		for (Car c : cars) {
			if (timer % 5 == 0) {
				...
				for (Car c2 : cars) {
					...
					double testCarY = c2.getY();
					if (distance(nextXSpot, nextYSpot, testCarX, testCarY) <= 25) {
						canMove = false;
					}
				}
				if (points != null) {
				...
				if (c.getX() > 300 && c.getX() < 700 && c.getY() > 300 && c.getY() < 700) {
	...
	public void removeOutOfBounds() {
		for (int i = 0; i < cars.size(); i++) {
			if (cars.get(i).getX() > 1100 || cars.get(i).getX() < -100 || cars.get(i).getY() > 1100
					|| cars.get(i).getY() < -100) {
				if (timer > 5000) {
					waitTimesCompiled.add(cars.get(i).waitTime);
				}
	...
	public void lightSwitchTimer() {
		...
		/*
		 * if(timer == 4000) { points = trafficController.stopCars(); blockers =
		 * points.get(0); greenLights = points.get(1); } if(timer == 6000) { points =
		 * trafficController.calculateMode(lanes); blockers = points.get(0); greenLights
		 * = points.get(1); timer = 0; }
		 */
